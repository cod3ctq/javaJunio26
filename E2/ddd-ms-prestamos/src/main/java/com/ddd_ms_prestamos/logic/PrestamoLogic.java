...
import com.ddd_ms_prestamos.mensajes.Mensajes;
...
public class PrestamoLogic implements PrestamoService {
    ...
    public PrestamoResponse calculaPrestamo(PrestamoRequest request) {
        //flujo:
        /*
        buscar la cuenta por clienteId
        buscar el tipo prestamo por id
        validar si es posible el prestamo
        transformar (entidad->response) con mapper
        retornar response
         */
    ...
    public PrestamoResponse registrarPrestamo(PrestamoRequest request) {
        ...
        buscar el tipo prestamo por id
        validar si es posible el prestamo
        transformar (request->entidad) con mapper
        crear la entidad
        persisitir
        transformar (entidad->response) con mapper
        retornar response
        ...
        if(envCuenta.isEmpty()){
            ...
            Prestamo prestamo = PrestamoMapper.toEntity(request.clienteId(),
                    request.tipoPrestamoId(), request.monto(),envTipo.get().getPlazo(), (double) datos.get(1));
