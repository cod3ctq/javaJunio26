...
public class PrestamoMapper {
    ...
    public static PrestamoResponse toResponse(Cuenta cuenta, TipoPrestamo tipo, double monto, String status){
        ...
        if(flag){
            response = new PrestamoResponse(tipo.getInteres(),monto,cantidadAPrestar,(cantidadAPrestar/tipo.getPlazo()),tipo.getPlazo(), LocalDate.now(),status);
        }
        return  response;
    }
    //Los argumentos recibidos del request
