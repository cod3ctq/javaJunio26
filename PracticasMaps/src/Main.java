...
public class Main {
    public static void main(String[] args) {
        ...
        //TIP Obtener un valor del mapa
        System.out.println(registro.get("00001111ABD"));

        //TIP quitar un valor del mapa
        registro.remove("00001111ABD");

        // quita el elemento solo si la llave esta asociada

        registro.remove("00001111ABD", "Susuna Gomez Flores 26/05/2000 EdoMex");

        //Exite el llave ?
        System.out.println(registro.containsKey("0001111ABD"));
        //Existe el valor ?
...
//
//            if (registro.get(key).getApellidoPat().endsWith("ez") && registro.get(key).getLugarOringen().startsWith("C")) {
//                System.out.println(registro.get(key));
//            }
//        }

        String tempKey = "20/11/2005";
        int startIndex = 3; // posición inicial (inclusive)
        int endIndex = 5;   // posición final (exclusive)
        String mes = tempKey.substring(startIndex, endIndex);
        System.out.println("Subcadena de índice " + startIndex + " a " + endIndex + ": " + mes);

        for (Persona p: registro.values()){
            if(p.getApellidoPat().endsWith("ez") && p.getLugarOringen().startsWith("C")){
