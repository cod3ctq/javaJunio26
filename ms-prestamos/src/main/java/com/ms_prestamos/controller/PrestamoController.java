...
public class PrestamoController {
    ...
    @PutMapping
    public ResponseEntity<Prestamos> actualizar(@RequestBody PrestamoRequest request){
        Prestamos prestamos = prestamo.actualizar(request);
        return new ResponseEntity<Prestamos>(prestamos, HttpStatusCode.valueOf(200));
    }

    @GetMapping("/buscar/{id}")
    public ResponseEntity<Prestamos> buscar(@PathVariable int id){
        Prestamos prestamos = prestamo.buscar(id);
        return new ResponseEntity<Prestamos>(prestamos, HttpStatusCode.valueOf(200));
    }
